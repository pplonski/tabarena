...
class OptunaCVLightGBM(AbstractExecModel):
    ...
    def _sample_params(self, trial: optuna.Trial) -> dict:
        ...
        params = {
            ...
            "subsample": 1.0, #trial.suggest_float("subsample", 0.7, 1.0),
    ...
    def _fit(self, X: pd.DataFrame, y: pd.Series, **kwargs):
        ...
        for fold_id, (train_idx, val_idx) in enumerate(split_indices):
            ...
            gf = GoldenFeatures(random_state=self.random_state + fold_id, verbose=0)
        ...
        self.use_golden_features_ = bool(study.best_trial.params.get("use_golden_features", True))
        self.fold_models = []
        self.fold_categorical_features = []
        self.fold_golden_features = []
        fold_tree_stats = []
        for split in prepared_splits:
            X_train = split["X_train"] if self.use_golden_features_ else split["X_train_raw"]
            y_train = split["y_train"]
            X_val = split["X_val"] if self.use_golden_features_ else split["X_val_raw"]
            y_val = split["y_val"]
            cat_cols = split["cat_cols"]
            if cat_cols:
                X_train = X_train.copy()
            ...
            self.fold_golden_features.append(split["gf"] if self.use_golden_features_ else None)
...
if __name__ == "__main__":
    expname = str(Path(__file__).parent / "experiments" / "quickstart_custom_model_lgbm_optuna_cv8_golden")
    leaderboard_dir = str(Path(__file__).parent / "leaderboards" / "quickstart_custom_model_lgbm_optuna_cv8_golden")
    ignore_cache = True
    ...
    methods = [
        Experiment(
            name="MyCustomModel_LGBM_OptunaCV8_Golden",
